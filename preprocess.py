...
def tokenize_examples(examples, tokenizer):
  ...
  for i in range(len(examples)):
    examples[i].word_list_a=[]
    examples[i].word_list_label=[]
    examples[i].keep_index=[]
    ex_labels=examples[i].label.split()
    if examples[i].label=="unsup":
      ex_labels = ["unsup" for i in range(len(examples[i].text_a.split()))]
    for index,w in enumerate(examples[i].text_a.split()):
        if w=="B-TRI" or w=="E-TRI":
          now=[w]
          examples[i].keep_index.extend([0 for i in range(len(now))])
        else:
          now=tokenizer.tokenize(w)
          examples[i].keep_index.extend([1 for i in range(len(now))])

        examples[i].word_list_a.extend(now)
        for jj in range(len(now)):
          if jj==0:
            examples[i].word_list_label.append(ex_labels[index])
          else:
            examples[i].word_list_label.append("x")
    if i % 10000 == 0:
...
def convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    tokens_a = example.word_list_a
    tokens_label = example.word_list_label
    input_type_ids = example.keep_index
    # tokens_b = None
    #if example.text_b:
    ...
    if len(tokens_a) > seq_length - 2:
      if trunc_keep_right:
        ...
        tokens_label = tokens_label[-(seq_length - 2):]
        input_type_ids = input_type_ids[-(seq_length - 2):]
      else:
        tokens_a = tokens_a[0:(seq_length - 2)]
        tokens_label = tokens_label[0:(seq_length - 2)]
        input_type_ids = input_type_ids[0:(seq_length - 2)]
    assert len(tokens_a)==len(tokens_label)==len(input_type_ids)
    ...
    tokens_a.append("[SEP]")
    input_type_ids.insert(0,1)
    input_type_ids.append(1)
    tokens_label.insert(0,"[CLS]")
    tokens_label.append("[SEP]")
    ...
    while len(input_ids) < seq_length:
      ...
      tokens_label.append("PAD")
   
    for label in tokens_label:
      if label not in label_map:
        print("**************:",tokens,tokens_label)
        break
    label_ids = [label_map[label] for label in tokens_label]
    ...
    #input_type_ids = np.concatenate((np.where(np.equal(1,input_type_ids))[0],np.where(n
    if ex_index < 1:
      ...
      tf.logging.info("label: %s" % " ".join([str(x) for x in tokens_label]))
...
def proc_and_save_unsup_data(
  ...
  if sub_set == "train":
    ...
    ori_examples = processor.get_unsup_examples(raw_data_dir, sub_set+"_ori")
    aug_examples = processor.get_unsup_examples(raw_data_dir, sub_set+"_aug")
  else:
    assert False
...
def main(_):
  ...
  if FLAGS.data_type == "sup":
    ...
    unsup_out_dir = FLAGS.output_base_dir
