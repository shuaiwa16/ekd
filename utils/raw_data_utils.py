...
class IMDbProcessor(DataProcessor):
  ...
  def get_unsup_examples(self, raw_data_dir, unsup_set):
    """See base class."""
    if unsup_set == "unsup_ext_ori":
      return self._create_examples(
          self._read_tsv(os.path.join(raw_data_dir, "unsup_ext_ori.csv"),
                         quotechar='"'), "unsup_ext", skip_unsup=False)
    elif unsup_set == "unsup_ext_aug":
      return self._create_examples(
          self._read_tsv(os.path.join(raw_data_dir, "unsup_ext_aug.csv"),
                         quotechar='"'), "unsup_ext", skip_unsup=False)
  ...
  def get_labels(self):
    """See base class."""
    ll= ['NEGATIVE','Conflict-Attack','Movement-Transport','Life-Die','Contact-Meet','Personnel-End-Position','Transaction-Transfer-Money','Personnel-Elect','Life-Injure','Transaction-Transfer-Ownership','Contact-Phone-Write','Personnel-Start-Position','Justice-Trial-Hearing','Justice-Charge-Indict','Justice-Sentence','Justice-Arrest-Jail','Life-Marry','Conflict-Demonstrate','Justice-Sue','Justice-Convict','Life-Be-Born','Business-Start-Org','Justice-Release-Parole','Justice-Appeal','Business-Declare-Bankruptcy','Business-End-Org','Justice-Fine','Life-Divorce','Justice-Execute','Business-Merge-Org','Personnel-Nominate','Justice-Extradite','Justice-Acquit','Justice-Pardon',"x","[CLS]","[SEP]","PAD","B-TRI","E-TRI","unsup"]
    tf.logging.info("*************label_size %d",len(ll))
    return ll
  ...
  def get_train_size(self, raw_data_dir):
    file_name=os.path.join(raw_data_dir,"train.csv")
    tf.logging.info("*************file_name %s",file_name)
    tl=len(self._read_tsv(os.path.join(raw_data_dir,"train.csv"),
                       quotechar='"'))-1
    tf.logging.info("*************train_size %d",tl)
    return tl

  def get_dev_size(self, raw_data_dir):
    dl=len(self._read_tsv(os.path.join(raw_data_dir,"test.csv"),
                       quotechar='"'))-1
    tf.logging.info("*************test_size %d",dl)
    return dl
