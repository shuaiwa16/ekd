...
from bert import memory_saving_gradients
...
from bert import optimization_hvd as optimization
...
def create_model(
  ...
  if unsup_ratio > 0:
    with tf.variable_scope("unsup_loss"):
      ...
      aug_input_type_ids_trans=tf.py_func(my_numpy_func,[p],Tout=tf.int64)
      aug_log_probs_middle = tf.gather_nd(params=aug_log_probs_before,indices=aug_input_type_ids_trans)
      aug_log_probs = tf.reshape(aug_log_probs_middle,[-1,max_seq_length,hidden_dim])
      ...
      if FLAGS.uda_softmax_temp != -1:
        tf.logging.info("*** **************************************************************************uda_softmax_temp  ***")
        m_aug_log_probs = tf.nn.log_softmax(
            aug_log_probs / FLAGS.uda_softmax_temp,
            axis=-1)
      ...
      tf.logging.info("*** **************************************************************************8Features ***") 
  else:
    tf.logging.info("*** **************************************************************************We have the issue here ***")
    unsup_loss = sup_loss
  ...
  return (sup_loss, unsup_loss, clas_logits[:sup_batch_size],
          ...
          tsa_threshold, unsup_loss_mask, correct_label_probs, tgt_aug_log_probs, ori_log_probs)
...
def model_fn_builder(
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (sup_loss, unsup_loss, logits,
     per_example_loss, loss_mask,
     tsa_threshold,
     unsup_loss_mask, correct_label_probs, tgt_aug_log_probs, ori_log_probs) = create_model(
    ...
    total_loss = sup_loss
    tf.logging.info("**** Sup Loss = %s ****",sup_loss)
    if unsup_ratio > 0 and uda_coeff > 0 and "input_ids" in features:
      ...
      #metric_dict["ori/ids"] = features["ori_input_ids"]
      #metric_dict["aug/ids"] = features["aug_input_ids"]
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      logging_hook = tf.train.LoggingTensorHook({"unsup loss" : unsup_loss}, every_n_iter = 200)
      ...
      def clas_metric_fn(per_example_loss, label_ids, logits):
        ...
        #unsup_loss = tf.metrics.mean(per_example_kl_loss)
        predictions = tf.argmax(logits, axis=-1, output_type=tf.int32)
        #accuracy = tf.metrics.accuracy(label_ids, predictions)
        precision = tf_metrics.precision(label_ids,predictions,41, list(range(1, 34)),average="micro")
        recall = tf_metrics.recall(label_ids,predictions,41,list(range(1, 34)),average="micro")
        f = tf_metrics.f1(label_ids,predictions,41,list(range(1, 34)),average="micro")
      ...
      predictions = tf.argmax(logits, axis=-1, output_type=tf.int32)
      output_spec = tf.contrib.tpu.TPUEstimatorSpec(
          mode = mode,predictions={"predict":predictions,"input_ids":input_ids,"label_ids":label_ids}, scaffold_fn=scaffold_fn
      )
