...
flags.DEFINE_string(
    "raw_data_dir", None,
    help="The input data dir of the original data. Should contain"
...
def main(_):
  hvd.init()
  FLAGS.model_dir = FLAGS.model_dir if hvd.rank() == 0 else os.path.join(FLAGS.model_dir, str(hvd.rank()))
  ...
  if FLAGS.do_train and FLAGS.do_eval:
    ...
    tf.logging.info("  training size = %d",
                    train_size)
    ...
    for _ in range(0, FLAGS.num_train_steps, save_checkpoints_steps):
      ...
      train_result = estimator.evaluate(input_fn=train_eval_input_fn, steps=train_steps)
    ...
    with open("label_test.txt",'w') as writer:
        for re in result:
            ...
            for gold_index,gold_item in enumerate(prediction):
              if gold_item>=34:
                gold[gold_index]=0
            for w,gold_label,label in zip(tokenizer.convert_ids_to_tokens([int(s) for s in sentence]),[id2label[id] for id in gold],[id2label[id] for id in prediction]):
              if w=="[PAD]":
                continue
              #if label=="NEGATIVE":
              #  continue
